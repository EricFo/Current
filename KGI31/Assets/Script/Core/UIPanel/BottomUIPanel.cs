...
namespace SlotGame.UI {
    ...
    public class BottomUIPanel {
        ...
        public void OnSpinBtnClick() {
            ...
            if (GlobalObserver.CurrGameState.Equals(GameState.Base))
            {
                AudioManager.PlayOneShot("SpinBtn");
            }
            else
            {
                AudioManager.PlayOneShot("FE_BonusSpin");
            }
        }
        /// <summary>
        /// Stop按钮点击事件
        ...
        public void IncressAward(int value, float time)
        {
            WinTxt.ScoreIncressTime(value, time);
        }
        /// <summary>
        /// 停止奖励动画，直接显示中奖额度
        /// </summary>
