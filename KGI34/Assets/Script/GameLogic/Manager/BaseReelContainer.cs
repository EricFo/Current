...
public class BaseReelContainer : ReelContainer
{
    ...
    public bool isViewWild = false;
    public bool isUpgradeWild=false;
    ...
    private float[][] ResultProbability;
    ...
    public override void Spin()
    {
        ...
        foreach (var reel in reels)
        {
            ...
            if(reel.GetVisibleSymbols()[0].ItemName is SymbolNames.WILD)
            {
                (reel.GetVisibleSymbols()[0] as WildSymbol).UpSortingOrder(false);
            }
        }

        ResultProbability = Random.Range(0, 1f) < ConfigManager.baseConfig.ResultChooseProbability ? ConfigManager.baseConfig.ResultScatterSymbolsProbability : ConfigManager.baseConfig.ResultNoScatterSymbolsProbability;
        ...
        reelsPredict[new List<int> { 1, 6, 11 }.GetRandomItem()].resultSymbols[0] = ConfigManager.baseConfig.ResultSymbols[GlobalObserver.GetRandomWeightedIndex(ResultProbability[6])];
        reelsPredict[new List<int> { 2, 12 }.GetRandomItem()].resultSymbols[0] = ConfigManager.baseConfig.ResultSymbols[GlobalObserver.GetRandomWeightedIndex(ResultProbability[7])];
        List<int> reelID = new List<int>() { 3, 8, 13 };
        int index = reelID.GetRandomItem();
        reelsPredict[index].resultSymbols[0] = ConfigManager.baseConfig.ResultSymbols[GlobalObserver.GetRandomWeightedIndex(ResultProbability[8])];
        reelID.Remove(index);
        reelsPredict[reelID.GetRandomItem()].resultSymbols[0] = ConfigManager.baseConfig.ResultSymbols[GlobalObserver.GetRandomWeightedIndex(ResultProbability[9])];
        reelsPredict[new List<int> { 4, 9, 14 }.GetRandomItem()].resultSymbols[0] = ConfigManager.baseConfig.ResultSymbols[GlobalObserver.GetRandomWeightedIndex(ResultProbability[10])];
        reelsPredict[7].resultSymbols[0] = ConfigManager.baseConfig.ResultSymbols[GlobalObserver.GetRandomWeightedIndex(ResultProbability[5])];
        ...
        foreach (var item in reelsPredict)
        {
            ...
            {
                System.Array.Copy(ConfigManager.cheatConfig.GetCheats(Cheat.TriggerBonusGame + "0"/*Random.Range(0, 5).ToString()*/, item.Key), 0, item.Value.resultSymbols, 0, 1);
            }
        ...
        //bool isFirstCoin = true;
        foreach (var item in reelsPredict)
        {
            if (item.Value.resultSymbols[0] == SymbolNames.COIN)
            {
                item.Value.coinLevel = 3;
                item.Value.coinJPType = GlobalObserver.IDJackpotDic[GlobalObserver.GetRandomWeightedIndex(ConfigManager.baseConfig.JackpotProbability[item.Key % 5 - 1])];
        ...
        if (Cheat.isGrand)
        {
            ...
            reelsPredict[8].coinScore = (int)(GlobalObserver.Meters[0] * 100);
        ...
        {
            reelsPredict[8].coinLevel = 3;
            reelsPredict[8].coinScore = (int)(GlobalObserver.Meters[1] * 100);
        ...
        {
            reelsPredict[8].coinLevel = 3;
            reelsPredict[8].coinScore = (int)(GlobalObserver.Meters[2] * 100);
        ...
        {
            reelsPredict[8].coinLevel = 3;
            reelsPredict[8].coinScore = (int)(GlobalObserver.Meters[3] * 100);
        ...
        if (false/*(reelsPredict[7].resultSymbols[0] == SymbolNames.SCATTER ? totalCoinCredit : 0) > GlobalObserver.BetValue * ConfigManager.baseConfig.ReelReverseBetMultiplier && Random.Range(0, 1f) < ConfigManager.baseConfig.ReelReverseProbability*/)
        {
            isReverse = true;
    ...
    protected override void OnReelStop(ReelBase reel)
    {
        ...
        if (reel.GetVisibleSymbols()[0].ItemName == SymbolNames.COIN)
        {
            isViewCoin = true;
    ...
    protected override ReelSpinArgs Predict(int id)
    {
        ...
        reelSpinArgs.resultSymbols[0] = ConfigManager.baseConfig.ResultSymbols[GlobalObserver.GetRandomWeightedIndex(ResultProbability[id % 5])];
        //要作弊就直接替换下最终结果
