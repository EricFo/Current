namespace SlotGame.Core.Reel
{
    ...
    public class ReelBase : MonoBehaviour, IReelState
    {
        ...
        public bool IsKeepRolling { get; set; }
        /// <summary>
        /// 当前Reel设置
        /// </summary>
        ...
        public virtual void Spin(ReelSpinArgs args)
        {
            ...
            maxMoveStep = setting.moveStep + (args.isHyper ? args.hyperStep : 0) + (args.isLong ? args.longStep : 0) + (args.isFreeStart ? args.freeStartStep : 0);
            if (args.isFreeStart)
            {
                transform.DOLocalMove(endPoint, setting.moveSpeed).SetRelative().SetEase(setting.beginCurve).OnComplete(Rolling);
            }
            else
            {
        ...
        public virtual void ReverseSpin(ReelSpinArgs args)
        {
            ...
            maxMoveStep = /*setting.moveStep + (args.isHyper ? args.hyperStep : 0) + (args.isLong ? args.longStep : 0)*/4;
        ...
        protected virtual void Rolling()
        {
            ...
            if (currMoveStep < maxMoveStep)
            {
                ...
                else
                {
                    ...
                    if (currMoveStep >= setting.moveStep + reelSpinArgs.WaitStep + (reelSpinArgs.isLong ? reelSpinArgs.longStep : 0) + (reelSpinArgs.isFreeStart ? reelSpinArgs.freeStartStep : 0) && !isShutDown)
                    {
        ...
        protected virtual void ReverseRolling()
        {
            ...
            if (currMoveStep < maxMoveStep)
            {
                ...
                transform.DOLocalMove(endPoint, speed * 6).SetRelative().SetEase(setting.reverseCurve).OnComplete(delegate ()
                {
                    AudioManager.Playback("FE_ReelBack");
                    ReverseRolling();
                });
                /*}*/
            }
        ...
        protected virtual CommonSymbol GetNextSymbol()
        {
            ...
            {
                symbolName = setting.reelStripes[id];
                if (symbolName == SymbolNames.SCATTER && reelSpinArgs.isReverse)
                {
