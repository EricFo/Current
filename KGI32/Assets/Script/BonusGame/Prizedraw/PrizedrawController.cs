...
public class PrizedrawController : MonoBehaviour
{
    ...
    public void InitBonus(int totalCoinCount)
    {
        ...
        InitPrizedrawResult(totalCoinCount);
    ...
    private void InitPrizedrawResult(int totalCoinCount)
    {
        int creditCount = GlobalObserver.GetRandomWeightedIndex(ConfigManager.bonusConfig.BonusIntroCreditCountProbability);
        int extraSpinCount = GlobalObserver.GetRandomWeightedIndex(ConfigManager.bonusConfig.BonusIntroExtraSpinCountProbability);
        ...
        int coinCount = ConfigManager.bonusConfig.BonusIntroCoinCount[GlobalObserver.CoinPosLevel.Count][index];
        ...
        for (int i = 0; i < extraSpinCount; i++)
        {
        ...
        for (int i = 0; i < coinCount; i++)
        {
            ...
            {
                case 0:
                    level = GlobalObserver.GetRandomWeightedIndex(ConfigManager.bonusConfig.BonusIntroCoinLevelProbability[coinCount > 1 ? 1 : 0]) + 1;
                    break;
                case 1:
                    level = GlobalObserver.GetRandomWeightedIndex(ConfigManager.bonusConfig.BonusIntroCoinLevelProbability[2]) + 1;
                    break;
                default:
                    level = 1;
                    break;
            }
            prizedrawResults.Add(levelCoinDic[level]);
            if (level == 3)
            {
                initLevel3CoinCount++;
            }
        ...
        //测试
        /*prizedrawResults.Clear();
        prizedrawResults.Add(PrizedrawResult.Coin1);
        prizedrawResults.Add(PrizedrawResult.Coin1);
        prizedrawResults.Add(PrizedrawResult.Coin2);
        prizedrawResults.Add(PrizedrawResult.Coin3);*/
    ...
    IEnumerator OnPickTigger(TurnSymbol item)
    {
        ...
        switch (prizedrawResults[currentSpin])
        {
            ...
            case PrizedrawResult.Coin1:
                ...
                break;
            case PrizedrawResult.Coin2:
                item.PlayCoinAnim(2);
                yield return new WaitForSeconds(18 / 30f);
                AudioManager.Playback("FE_BagPickCoin");
                yield return new WaitForSeconds(22 / 30f);
                item.ResetCoin();
                OnCoinClickEvent.Invoke(2);
                yield return new WaitForSeconds(0.5f);
                break;
            case PrizedrawResult.Coin3:
                item.PlayCoinAnim(3);
                yield return new WaitForSeconds(18 / 30f);
                AudioManager.Playback("FE_BagPickCoin");
                yield return new WaitForSeconds(22 / 30f);
                item.ResetCoin();
                OnCoinClickEvent.Invoke(3);
                yield return new WaitForSeconds(0.5f);
                break;
