...
public class BaseReelContainer : ReelContainer
{
    ...
    public override void Spin()
    {
        ...
        foreach (var item in reelsPredict)
        {
            ...
            {
                System.Array.Copy(ConfigManager.cheatConfig.GetCheats(Cheat.TriggerBonusGame, item.Key), 0, item.Value.resultSymbols, 0, 1);
            }
        ...
        foreach (var item in reelsPredict)
        {
            if (item.Value.resultSymbols[0] == SymbolNames.COIN)
            {
                ...
                if (item.Value.coinLevel == 3 && index2 > (ConfigManager.baseConfig.CoinValueByLevel[1][3].Length - 4 - 1))
                {
                    //item.Value.coinScore = ConfigManager.baseConfig.CoinValueByLevel[GlobalObserver.GetMultiplyer()][item.Value.coinLevel][index2];
                    item.Value.coinJPType = GlobalObserver.IDJackpotDic[index2 - (ConfigManager.baseConfig.CoinValueByLevel[1][3].Length - 4)];
                }
